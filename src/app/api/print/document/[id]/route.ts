...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: 'DM Sans', sans-serif; background: #f8fafc; color: #0f172a; font-size: 13px; line-height: 1.6; }
    ...
    .header { background: linear-gradient(135deg, #0284c7 0%, #0891b2 100%); padding: 28px 40px; color: white; display: flex; justify-content: space-between; align-items: flex-start; }
    .clinic-name { font-family: 'Playfair Display', serif; font-size: 22px; font-weight: 600; }
    .doctor-info { font-size: 12px; opacity: 0.85; margin-top: 4px; line-height: 1.6; }
    .header-right { text-align: right; font-size: 11px; opacity: 0.8; line-height: 1.7; }
    .body { padding: 40px; flex: 1; }
    .doc-title { font-family: 'Playfair Display', serif; font-size: 20px; font-weight: 600; text-align: center; color: #0284c7; margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.05em; }
    .doc-date { text-align: center; font-size: 12px; color: #64748b; margin-bottom: 32px; }
    .divider { width: 60px; height: 3px; background: #0284c7; margin: 0 auto 32px; border-radius: 2px; }
    .content { font-size: 14px; line-height: 1.8; color: #1e293b; }
    .content p { margin-bottom: 12px; }
    .content strong { color: #0f172a; }
    .signature-area { padding: 32px 40px; display: flex; justify-content: flex-end; }
    ...
    .footer { background: #f8fafc; padding: 14px 40px; border-top: 1px solid #e2e8f0; text-align: center; font-size: 10px; color: #94a3b8; }
    .stamp-area { margin-top: 40px; }
    .stamp { width: 100px; height: 100px; border: 2px dashed #94a3b8; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 10px; color: #94a3b8; text-align: center; padding: 8px; }
  ...
  <div class="page">
    <div class="header">
      <div>
        ...
        <div class="doctor-info">${doctorName}${specialty ? '<br/>' + specialty : ''}${address ? '<br/>' + address : ''}${phone ? '<br/>' + phone : ''}</div>
      </div>
      <div class="header-right">
        <div>Date: ${docDate}</div>
        <div style="opacity:0.6;font-size:10px;font-family:monospace;margin-top:4px;">DOC-${doc.id.slice(-8).toUpperCase()}</div>
      </div>
    </div>
    <div class="body">
      ...
      <div class="content">${getDocContent()}</div>
      <div class="stamp-area"><div class="stamp">Official<br/>Stamp</div></div>
    </div>
    ...
    <div class="footer">${clinicName} · ${address} · ${phone}</div>
  </div>
